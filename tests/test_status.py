...
def test_id_bre():
    ...
    for i in response_bre.json():
        bre_id = i["id"]
        response = requests.get(f"https://api.openbrewerydb.org/v1/breweries/{bre_id}")
...
def test_city_bre():  # Тестовый запрос про пивоварни в каждом городе
    ...
    for i in unique_city:
        response = requests.get(
            f"https://api.openbrewerydb.org/v1/breweries?by_city={i}"
        )
...
def test_country_bre():  # Тестовый запрос про пивоварни в каждой стране
    ...
    for i in unique_country:
        response = requests.get(
            f"https://api.openbrewerydb.org/v1/breweries?by_country={i}"
        )
...
def test_bre1():
    ...
    response = requests.get(
        "https://api.openbrewerydb.org/v1/breweries/autocomplete?query=texas",
        params=param,
    )
    assert response.headers["Content-Encoding"] == "gzip"
    assert response.headers["Connection"] == "keep-alive"
...
def test_bre2():
    ...
    response = requests.get(
        "https://api.openbrewerydb.org/v1/breweries?by_country=south%20korea",
        params=param,
    )
    assert response.headers["Content-Encoding"] == "gzip"
    assert response.headers["Connection"] == "keep-alive"
...
def test_bre5():
    response = requests.get(
        " https://api.openbrewerydb.org/v1/breweries/meta?by_type=micro"
    )
    assert response.headers["Content-Encoding"] == "gzip"
    assert response.headers["Connection"] == "keep-alive"
    assert response.headers["Server"] == "cloudflare"
